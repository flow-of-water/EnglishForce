...
export const getAllProgramsWithProgressStatus = async userId => {
  ...
  `,
	...
	const lessonCountMap = Object.fromEntries(lessonCounts.map(row => [row.program_id, parseInt(row.total_lessons)]));

	// 2. Truy vấn số bài học đã học của user theo program_id
	const userProgressCounts = await db.UserProgress.findAll({
		where: { user_id: userId },
		attributes: ['program_id', [Sequelize.fn('COUNT', Sequelize.col('lesson_id')), 'learned']],
		group: ['program_id'],
	});
