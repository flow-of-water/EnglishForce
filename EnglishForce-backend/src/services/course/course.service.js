...
export const getPaginatedCourses = async (limit, offset, userId = null, q = '') => {
	...
	if (q?.length >= 1) {
		whereClause = {
			[Op.or]: [{ name: { [Op.iLike]: `%${q}%` } }, { instructor: { [Op.iLike]: `%${q}%` } }],
		};
	}
...
export const searchCourses = async query => {
	return await Course.findAll({
		where: {
			[Op.or]: [{ name: { [Op.iLike]: `%${query}%` } }, { author: { [Op.iLike]: `%${query}%` } }],
		},
		raw: true,
	});
};
...
export const deleteCourse = async id => {
	...
	if (course.thumbnail_public_id) await deleteCloudinaryFile(course.thumbnail_public_id, 'image');
	for (const section of course.CourseSections || []) {
		if (section.video_public_id) await deleteCloudinaryFile(section.video_public_id, 'video');
	}
...
export const getTopRatedCourses = async (k = 5) => {
	const courses = await Course.findAll({
		attributes: {
			include: [
				[fn('COALESCE', fn('AVG', col('UserCourses.rating')), 0), 'average_rating'],
				[fn('COUNT', col('UserCourses.rating')), 'total_rating'],
			],
		},
		include: [
			{
