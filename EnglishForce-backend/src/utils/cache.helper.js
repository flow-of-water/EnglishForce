...
export const cacheWrapper = {
	async create(createFn, invalidateKeys = []) {
		try {
			...
			console.error('❌ cacheWrapper.create error:', err.message);
			throw err;
		}
	},
	...
	async read(key, fetchFn) {
		try {
			...
			console.error('❌ cacheWrapper.read error:', err.message);
			throw err;
		}
	},
	...
	async update(updateFn, invalidateKeys = []) {
		try {
			...
			console.error('❌ cacheWrapper.update error:', err.message);
			throw err;
		}
	},
	...
	async delete(deleteFn, invalidateKeys = []) {
		try {
			...
			console.error('❌ cacheWrapper.delete error:', err.message);
			throw err;
		}
	},
};
