...
const ExamStartPage = () => {
		...
		try {
			...
			const extractQuestions = part => {
				part.Questions?.forEach(question => {
					const selected = question.Answers.filter(a => answers[question.public_id]?.includes(a.public_id));
	...
	const renderPartRecursively = (part, indexPath = '') => (
		<Box key={part.public_id} mb={5}>
			...
			{typeof part.description === 'string' && isHtml(part.description) ? (
			...
			{part.Questions?.map(question => {
				const rendered = (
					<Paper
						...
						elevation={3}
						sx={{ p: 3, my: 3, borderLeft: '6px solid #1976d2' }}
					>
			...
			{part.Children?.map((childPart, idx) => renderPartRecursively(childPart, `${indexPath}.${idx + 1}`))}
		</Box>
	);
	...
	return (
		<Box p={4}>
			...
			{exam.parts.map((part, index) => renderPartRecursively(part, `${index + 1}`))}
		</Box>
	);
};
