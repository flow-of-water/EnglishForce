	...
	return (
		<Container maxWidth="md" sx={{ py: 4 }}>
			<Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
				...
				<Button variant="outlined" onClick={() => navigate(`/admin/programs/${program.public_id}/edit`)}>
					Edit
				</Button>
			</Box>
			...
			<Box sx={{ border: '1px solid gray', p: 2, borderRadius: 2, mb: 4 }}>
				...
				<Button variant="contained" color="primary" onClick={handleCreateUnit} disabled={creating}>
			...
			{program.Units?.map((unit, index) => (
				<Card key={unit.public_id} sx={{ mb: 3 }}>
					<CardContent
						...
						onClick={() => navigate(`/admin/programs/${publicProgramId}/units/${unit.public_id}`)}
					>
						<Box>
							<Typography variant="h6" gutterBottom>
