	...
	return (
		<Container maxWidth="md" sx={{ py: 4 }}>
			<Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
				...
				<Button variant="outlined" onClick={() => navigate(`/admin/exercises/${exercise.public_id}/edit`)}>
					Edit
				</Button>
			</Box>
			...
			{exercise.thumbnail && (
				<Box sx={{ mb: 1 }}>
					...
					<img src={exercise.thumbnail} alt="Exercise Thumbnail" style={{ width: '100%', borderRadius: 4 }} />
				</Box>
			)}

			{exercise.record && <Typography variant="body2">Record: {exercise.record}</Typography>}
					...
					<FormControlLabel
						control={
							<Checkbox
								...
								onChange={e => setNewAnswer(prev => ({ ...prev, is_correct: e.target.checked }))}
							/>
						}
				...
				exercise.ExerciseAnswers.map(answer => (
					<Card key={answer.public_id} sx={{ mb: 2, position: 'relative' }}>
						...
						>
							...
							{answer.is_correct && <Chip label="Correct" color="success" size="small" sx={{ mt: 1 }} />}
						</CardContent>
						<IconButton
							color="error"
							size="small"
