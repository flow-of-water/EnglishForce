	...
	return (
		<Container maxWidth="md" sx={{ py: 4 }}>
			<Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
				...
				<Button variant="outlined" onClick={() => navigate(`/admin/lessons/edit/${lesson.public_id}`)}>
					Edit
				</Button>
			</Box>
			...
			<Box sx={{ mb: 4, p: 2, border: '1px solid gray', borderRadius: 2 }}>
				...
				<TextField
					...
					onChange={e => setNewSection({ ...newSection, order_index: parseInt(e.target.value, 10) })}
					fullWidth
					sx={{ mb: 2 }}
				/>
				...
				lesson.Exercises.map((ex, index) => (
					<Card key={ex.public_id} sx={{ mb: 2 }}>
						...
						>
							...
							<IconButton onClick={e => handleDeleteExercise(ex.public_id, e)} color="error">
								<DeleteIcon />
							</IconButton>
						</CardContent>
					</Card>
