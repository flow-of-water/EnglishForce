		...
		<Box sx={{ mt: 2 }}>
			{questions.map((q, qIdx) => (
				<Box key={q.id} sx={{ mb: 3 }}>
					...
					{q.thumbnail && (
						<img src={q.thumbnail} alt="thumbnail" style={{ width: 100, marginTop: 8, borderRadius: 4 }} />
					)}
					{q.record && (
		...
		<Accordion key={part.id} sx={{ mb: 2 }}>
			<AccordionSummary expandIcon={<ExpandMoreIcon />}>
				<Box display="flex" justifyContent="space-between" alignItems="center" width="100%">
					...
					<Box>
						<IconButton
							color="info"
							onClick={() => navigate(`/admin/exams/${publicId}/parts/${part.public_id}`)}
						>
							<Visibility />
						</IconButton>
						<IconButton
			...
			<AccordionDetails>
				...
				{part.Children && part.Children.length > 0 && (
					<Box sx={{ mt: 2, ml: 2 }}>{part.Children.map(child => renderPartRecursive(child))}</Box>
				)}
			</AccordionDetails>
		</Accordion>
	);
		...
		return (
			<Container sx={{ mt: 4 }}>
				<Typography variant="h6" color="error">
					Exam not found or no parts available.
				</Typography>
			</Container>
		);
	}
		...
		<Container sx={{ mt: 4 }}>
			...
			<Paper sx={{ p: 3, mb: 4 }}>
				...
				<TextField
					...
					sx={{ mb: 2 }}
				/>
				<Button variant="contained" onClick={handleCreatePart}>
					Add Part
				</Button>
			</Paper>
