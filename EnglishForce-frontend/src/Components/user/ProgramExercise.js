...
const ExerciseCard = ({ exercise, index, total, onAnswerChecked, handleNext, isLast }) => {
	...
	const renderAnswerSection = () => {
		switch (exercise.type) {
			...
			case 'speaking':
				return (
					<Box mt={2}>
						<SpeakingExercise expectedText={exercise.question} onResult={setSpeakingResult} />
					</Box>
				);
			default:
		...
		>
			...
			<CardContent>
				<Stack spacing={2}>
					...
					<Typography sx={{ whiteSpace: 'pre-line', fontSize: '1.1rem' }}>{exercise.question}</Typography>
					...
					{showResult && (
						<>
							...
							{exercise.explanation && (
								<Alert severity="info">📘 Explanation: {exercise.explanation}</Alert>
							)}
						</>
					)}
					...
					{!showResult ? (
						...
						<Button variant="outlined" onClick={handleNext} sx={{ alignSelf: 'flex-end' }}>
